...
def knapsack_dp(profit, weight, capacity):
    ...
    for item_id in range(1, N):  # item
        for capacity_cells in range(1, M + 1):  # capacity cell (extra 1 at the beginning)
            ...
            if remaining_cells >= 0:  # item fits or even has extra remaining cells (take from same row!!!)
                include = profit[item_id] + dp[item_id][remaining_cells]  # Check the same row if items fits multiple times!
